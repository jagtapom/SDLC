...
@ui.page("/")
def main():
    state = app.storage.user  # ✅ now valid inside page builder
    logger = logging.getLogger(__name__)

    # Set defaults if not already set
    state.setdefault("workflow_status", "initial")
    state.setdefault("chat_manager", None)
    state.setdefault("uploaded_file_path", None)
    state.setdefault("stories_approved", False)
    state.setdefault("stories_file", None)
    state.setdefault("code_approved", False)
    state.setdefault("code_file", None)
    ...
    with ui.row().classes("w-full"):
        ...
        with ui.column().classes("w-3/4 p-8 bg-gray-50 min-h-screen"):
            ...
            with ui.card().classes("bg-white shadow-md p-6 mb-6"):
                ...
                ui.button("PROCESS REQUIREMENTS", on_click=lambda: process_requirements(state)).props('color="primary"').classes("mt-4")
...
def process_requirements(state):
